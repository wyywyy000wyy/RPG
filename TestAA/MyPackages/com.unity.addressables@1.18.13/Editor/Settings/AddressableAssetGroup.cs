        ...
        internal void ResetEntryMap()
        {
            ...
            foreach (var e in m_SerializeEntries)
            {
                try
                {
                    e.parentGroup = this;
                    e.IsSubAsset = false;
                    if(m_EntryMap.TryGetValue(e.guid,out var old))
                    {
                        var erro = $"ResetEntryMap Error:  ({old.address},{e.guid},{e.AssetPath},{e.parentGroup.name}) has already added to ({old.address},{e.guid},{old.AssetPath},{old.parentGroup.name}) ";
                        Debug.LogException(new Exception(erro));
                    }
