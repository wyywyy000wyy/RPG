...
namespace UnityEditor.AddressableAssets.Diagnostics.GUI
{
    class EventViewerWindow : EditorWindow, IComparer<EventDataSet>
    {
        ...
        public void OnEvent(DiagnosticEvent diagnosticEvent, int session)
        {
            if (pause)
                return;
            if(!string.IsNullOrEmpty(filter) && !diagnosticEvent.DisplayName.Contains(filter))
            {
                return;
            }
            if (m_EventData == null)
            ...
            if (diagnosticEvent.DisplayName == "FPS")
            {
                Debug.Log($"fps__ frame={diagnosticEvent.Frame} value={diagnosticEvent.Value}");
            }
        ...
        void DrawToolBar(EventDataPlayerSession session)
        {
            ...
            GUILayout.Label("¹ýÂË");
            filter = EditorGUILayout.TextField(filter, GUILayout.Width(249));
